...
\tabhead{Spalte} & \tabhead{Typ} & \tabhead{Pflicht} & \tabhead{Bedeutung} \\
\midrule
\texttt{id}        & Ganzzahl & ja, Primärschlüssel & Eindeutige interne Kennung der Person. Diese Kennung — nicht der Name — identifiziert die Person eindeutig. \\
\texttt{vorname}   & Text     & ja & Initiale(n) des Vornamens (z.\,B.~\textit{C.}, \textit{Ch.}, \textit{H.-P.}). Der ausgeschriebene Vorname wird nicht gepflegt. \\
\texttt{nachname}  & Text     & ja & Nachname in offizieller Schreibweise. \\
...
\smallskip
\textbf{Was, wenn zwei Personen wirklich den gleichen Namen haben?}
Das ist erlaubt. Wenn es zwei verschiedene Forscher mit der gleichen
Schreibung gibt (z.\,B.~zwei \textit{M.~Müller}), erhalten sie zwei
separate Zeilen mit unterschiedlichen Kennungen, jeweils mit ihren
eigenen Beiträgen, Schreibvarianten und Instituten. Die Kennung
\texttt{id} ist die eindeutige Identität, nicht der Name.
\end{hinweis}
