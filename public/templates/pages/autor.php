...
$stmt = $db->prepare('SELECT id, name, name_display FROM autoren WHERE id = ?');
...
$pageTitle    = $autor['name_display'] . ' - ' . SITE_NAME;
$canonicalUrl = canonicalUrl('/autor/' . $autor['id']);
$metaTags = [
    ['name' => 'description', 'content' => $autor['name_display'] . ' - ' . count($papers) . ' Beiträge in DGaO-Proceedings'],
];
?>
...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb small">
        ...
        <li class="breadcrumb-item active"><?= e($autor['name_display']) ?></li>
    </ol>
</nav>

<h1 class="h3 mb-1"><?= e($autor['name_display']) ?></h1>
